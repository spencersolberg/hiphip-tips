	...
	async GET(req, ctx) {
		...
		if (token) {
			try {
				...
				const walletsWithNames = wallets.map((wallet, i) => ({
					...wallet,
		...
		<>
			...
			<div class="p-4 mx-auto flex max-w-screen-xl flex-col text-white min-h-screen">
				...
				<div class="max-w-sm mx-auto w-full">
					<h1 class="text-4xl font-bold mt-8">your domains</h1>
				</div>
				...
				<div class="max-w-sm mx-auto w-full">
					<p class="text-xl text-center mt-4">
						...
						<span class="font-bold">
							{subdomain}.{Deno.env.get("HANDSHAKE_DOMAIN")}
						</span>
					</p>
					<p class="text-xl text-center mt-4">
						it is also accessible at{" "}
						<span class="font-bold">
							{subdomain}.{Deno.env.get("ICANN_DOMAIN")}
						</span>
					</p>
					<p class="text-xl text-center mt-4">
						<a class="underline hover:italic" href="/domains">
				...
				<div class="grid grid-cols-2 gap-4 max-w-sm mx-auto mt-8">
					<form method="post" action="/security">
						...
						<button
							type="submit"
							class={`w-full text-xl  mt-2 text-center underline hover:italic ${
								security === "handshake" ? "text-green-400" : "text-white"
							}`}
							value="handshake"
							name="submit"
						>
						...
						{security === "handshake" && (
							<p class="text-center text-sm">(active)</p>
						)}
					</form>
					<form method="post" action="/security">
						<input type="hidden" name="path" value="/account" />
						...
						<button
							type="submit"
							class={`w-full text-xl  mt-2 text-center underline hover:italic ${
								security === "ca" ? "text-green-400" : "text-white"
							}`}
							value="ca"
							name="submit"
						>
				...
				<div class="grid grid-cols-2 gap-4 max-w-sm mx-auto mt-8">
					{wallets?.length ? (
							...
							return (
								<form class="" method="post">
									...
									<CoinButton
										...
										domain={`${subdomain}.${Deno.env.get("HANDSHAKE_DOMAIN")}`}
										name={wallet.name}
										generic={wallet.name === "Unknown"}
									/>
									<button
										class="w-full text-xl  mt-2 text-center text-red-400 hover:underline hover:italic"
										type="submit"
										value="deleteSubdomainWallet"
										name="submit"
									>
										<p>delete</p>
									</button>
						...
						<p class="text-xl text-center mx-auto col-span-2">
				...
				<form class="mx-auto w-full flex flex-col max-w-sm" method="post">
					<label class="text-xl mt-4 -mb-6" for="newSubdomain">
						<p>change subdomain</p>
					</label>
					...
					{authenticators?.map((authenticator) => {
						return (
							<form class="w-full mb-3" method="post">
								...
								<input
									type="text"
									class="rounded-md w-full text-2xl px-4 pb-1 pt-0.5 w-full border-2 border-black text-black"
									name="newName"
									value={authenticator.name}
								/>
								<div class="grid grid-cols-3">
									<button
										class="w-full text-xl  mt-2 text-left text-green-400 hover:underline hover:italic"
										type="submit"
										value="renameAuthenticator"
										name="submit"
									>
										<p>rename</p>
									</button>
									<button
										class={`w-full text-xl  mt-2 text-center pr-2 text-red-400 hover:underline hover:italic ${
											authenticators.length > 1 ? "" : "hidden"
										}`}
										type="submit"
										value="deleteAuthenticator"
										name="submit"
									>
										<p>delete</p>
									</button>
									{authenticator.name === currentAuthenticator && (
										<p class="w-full text-xl  mt-2 text-right text-slate-400">
											(current)
										</p>
									)}
								</div>
				...
				<div class="max-w-sm mx-auto w-full mt-8">
					...
					<p class="mt-2">
						this link will sync your session from{" "}
						{currentDomain === "handshake" ? HANDSHAKE_DOMAIN : ICANN_DOMAIN} to{" "}
						{currentDomain === "handshake" ? ICANN_DOMAIN : HANDSHAKE_DOMAIN}
					</p>
				</div>
				<form class="mx-auto w-full flex flex-col max-w-sm mt-2" method="post">
					<a
						href="/logout"
						class="rounded-md w-full text-2xl px-4 pb-1 pt-0.5 mt-4 text-center border-2 border-black text-white bg-yellow-400 transition-transform transform-gpu hover:scale-110"
					>
						logout
					</a>
					<button
						class="rounded-md w-full text-2xl px-4 pb-1 pt-0.5 mt-2 text-center border-2 border-black text-white bg-red-400 transition-transform transform-gpu hover:scale-110"
						type="submit"
						value="deleteSubdomain"
						name="submit"
					>
						delete account
					</button>
