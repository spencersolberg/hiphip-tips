...
export const handler = async (
	...
	const origin = `${protocol}://${rpID}${
		host === "localhost" ? `:${Deno.env.get("PORT")}` : ""
	}`;
	...
	try {
		...
		console.error(error);
		return new Response("Internal server error. (Challenge probably expired)", {
			status: 500,
		});
	}
	...
	try {
		...
		console.error(error);
		return new Response(
			"Internal server error. (Authenticator not registered)",
			{ status: 500 },
		);
	}
	...
	if (verified) {
		...
		const token = await signToken(uuid, authenticator.name, rpID);
