...
export default function Key(props: { keyValue: string }) {
	...
	return (
		<>
			<p
				class={`text-lg mx-auto mt-4 text-slate-400 ${
					javascript
						? "cursor-pointer transition-transform transform-gpu hover:scale-105 hover:underline"
						: ""
				}`}
				onClick={() => {
					if (javascript) {
			...
			<p class={`${javascript ? "" : "hidden"} mx-auto mt-4 text-slate-400`}>
				click to copy
			</p>
		</>
	);
